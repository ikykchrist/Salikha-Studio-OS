...
type BookingRecord = { id: string; client: string; eventName: string; date: string; startTime: string; endTime: string; venue: string; packageName: string; total: number; paidAmount?: number; downpaymentAmount?: number; status: "PENDING" | "DONE" | "CANCELLED" };
...
export function BookingEditForm({ booking, onClose, onSave }: { booking: BookingRecord; onClose: () => void; onSave: (booking: BookingRecord) => void | Promise<void> }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try { await onSave({ ...booking, eventName: eventName.trim(), date, startTime, endTime, venue: venue.trim(), packageName: packageName.trim(), total: Number(total), downpaymentAmount: Number(downpayment), status }); }
