...
export function InvoicesView() {
  ...
  const saveLogo = async (logo: string | null) => {
    ...
    try {
      ...
      await showNotice(logo ? "This logo is now saved in the local database and will be used on invoices." : "The saved invoice logo has been removed.", logo ? "Invoice logo saved" : "Logo removed");
  ...
  return <>
    ...
    {!emailReady && <div className="invoice-email-setup"><Mail aria-hidden="true" /><span><strong>Email sending needs setup.</strong> Configure <code>RESEND_API_KEY</code> and <code>INVOICE_FROM_EMAIL</code> on this local app server. Creation, logo saving, and printing work independently.</span></div>}
